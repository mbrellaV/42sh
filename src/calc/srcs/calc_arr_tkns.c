...
static t_calc_tkns	*calc_new_tkns(void)
{
	...
	s_tokens->first_token = tokens;
	s_tokens->malloc_size = CALC_TOKENS_SIZE;
	s_tokens->size = 0;
	return (s_tokens);
}

static t_calc_tkns	*calc_dop(char *str, t_calc_tkns *s_tokens,
								size_t *pos)
{
	if (!str || !pos)
	...
	t_calc_tkns *s_tokens, size_t index)
{
	...
	while (1)
	{
		s_tokens = calc_dop(str, s_tokens, &pos);
		if (!s_tokens || !(token = calc_get_next_tkn(str, pos)))
			return (calc_del_tkns(s_tokens));
		ft_memcpy(&s_tokens->first_token[s_tokens->size], token, sizeof(t_calc_tkn));
		s_tokens->size++;
		pos = token->end_pos;
		free(token);
		if (s_tokens->first_token[s_tokens->size - 1].type == CALC_END)
			break ;
...
t_calc_tkns			*calc_del_tkns(t_calc_tkns *s_tokens)
{
	...
	while (++i < s_tokens->size)
		if (s_tokens->first_token[i].value)
			free(s_tokens->first_token[i].value);

	...
	t_calc_err *error)
{
	...
	sign = 1;
	if ((s_tokens->first_token)[*index].type == CALC_MINUS)
		sign = -1;
	(*index)++;
														...
														t_calc_err *error)
{
	...
	if (s_tokens->first_token[*index].type == CALC_NUMBER)
	{
		result = calc_ll_atoi((char *)s_tokens->first_token[*index].value);
		(*index)++;
		return (result);
	}
	else
														...
														t_calc_err *error)
{
	if (!s_tokens || !calc_check_index(s_tokens, *index))
		return (calc_error(error, CALC_ERR_NULL_OR_IND));
	if (s_tokens->first_token[*index].type == CALC_MINUS
		|| s_tokens->first_token[*index].type == CALC_PLUS)
		return (calc_unary_next(s_tokens, index, error));
	else if (s_tokens->first_token[*index].type == CALC_NUMBER)
